    ...
    <div className="container">
      ...
      <Table className="table table-hover table-bordered">
        ...
        <tr>
          ...
          <th scope="col">Price (€) <Button onClick={sortByLowestPrice}>{("Low to high")}</Button></th>
        ...
        {products.map(product => 
            <tr key={product.name + product.price}>
              ...
              <td className={ product.quantity < 3 ? 'low-quantity' : 'high-quantity'}>{product.quantity}</td> 
              <td>{product.store}</td>
            </tr>
          )}
